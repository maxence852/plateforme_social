...
@section('content')
    ...
    @foreach($thread->comments()->get() as $comment)
        <div class="well">
            <h4> By : {{$comment->author->name }} on {{$comment->created_at}} </h4> <!-- $author pointe dans ForumController  function thread($id) qui récupère le nom de l'utilisateur qui a écris le msg.-->
            <hr style="border-color: #d58512">
            <p>{{ nl2br(BBCode::parse($comment->body)) }}</p> <!-- todo bbcode ne fonctionne pas sur la page -->
            </hr>
            @if(Auth::check() && Auth::user()->isAdmin())
                <a href="{{ URL::route('forum-delete-comment',$comment->id)}}"  class="btn btn-danger">Delete</a>
            @endif
        </div>
